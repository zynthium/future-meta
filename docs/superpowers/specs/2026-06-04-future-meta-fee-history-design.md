...
let fee = meta.contract_fee_at("SHFE.cu2607", offset_date_time)?;
...
let fee = meta.contract_fee_for_handle_at(handle, offset_date_time)?;
...
```text
symbol -> ContractHandle
underlying_symbol -> Vec<ContractHandle>
ContractHandle.index -> sorted Vec<fee_version_id>
main_contract_by_underlying_symbol -> sorted versions or filtered query
```

单合约 as-of 查询有两条路径：

- 便利路径：`contract_fee_asof(symbol, at)` 接收 RFC3339 字符串，内部解析为 `OffsetDateTime` 后查询。
- 高频路径：调用方先用 `resolve_contract(symbol)` 得到 `ContractHandle`，再使用 `contract_fee_for_handle_at(handle, at)` 或 `contract_fee_for_handle_on(handle, trading_date)` 查询。

手续费盘中不变，因此 client 加载 archive 时可以派生交易所本地 `valid_from_date` / `valid_to_date` 内存索引。该索引只存在于 `FutureMeta` 运行时对象，不写入压缩 archive，也不违反“不存储派生字段”的持久化约束。`contract_fee_for_handle_on` 跳过字符串解析和 symbol hash 查找，是单合约重复查询的最快路径。

品种查询先从传入的 `underlying_symbol` 查合约集合，再对每个合约做 as-of 查询并按交易状态、上市日期、到期日期过滤。主连查询接收 `KQ.m@...` symbol，解析出 `underlying_symbol` 后返回该时间点的主力合约手续费版本。
...
- 禁止字段变化不影响 `rule_hash`。
- as-of 二分查询边界。
- `OffsetDateTime` 查询、交易日查询和 `ContractHandle` 热路径。
...
- 对比 `contract_fee_asof`、`contract_fee_at`、`contract_fee_on`、`contract_fee_for_handle_at`、`contract_fee_for_handle_on`。
